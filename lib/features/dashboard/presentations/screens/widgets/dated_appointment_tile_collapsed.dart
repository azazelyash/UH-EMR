...
import 'package:aasa_emr/features/dashboard/presentations/screens/patient_info_screen.dart';
import 'package:aasa_emr/features/dashboard/presentations/providers/appointment_provider.dart';
import 'package:aasa_emr/features/dashboard/presentations/screens/widgets/indicator_chip.dart';

class DatedAppointmentTileCollapsed extends StatefulWidget {
  const DatedAppointmentTileCollapsed({super.key, required this.appointment});

  final Appointment appointment;

  @override
  State<DatedAppointmentTileCollapsed> createState() => _DatedAppointmentTileCollapsedState();
}

class _DatedAppointmentTileCollapsedState extends State<DatedAppointmentTileCollapsed> {
  @override
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                Flexible(
                  flex: 4,
                  child: IndicatorChip(
                    title: (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.done)
                        ? AppointmentStatusValues.done
                    ...
                    showButton: false,
                    color: (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.done)
                        ? Colors.green
                        : (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.pending)
                            ? Colors.blue
                            : Colors.red,
                    icon: (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.done)
                        ? Icons.check_rounded
                        : (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.pending)
                            ? Icons.pending_actions_rounded
                            : Icons.close_rounded,
                    backgroundColor: (widget.appointment.status!.appointmentStatus == AppointmentStatusValues.done)
